...
$armasigma = new Login($db);

$data = json_decode(file_get_contents("php://input"));

$armasigma->email = $data->email;
$armasigma->senha = $data->senha;
...
em formato de array
Assim será mais fácil de adcionar no array de armasigmas para apresentar 
ao final
*/
    while($linha = $rs->fetch(PDO::FETCH_ASSOC)){
        ...
        */
        ...
        array_push($armasigma_arr["saida"],$array_item);

    }

    header("HTTP/1.0 200");
    echo json_encode($armasigma_arr);
}
else{
    header("HTTP/1.0 400");
